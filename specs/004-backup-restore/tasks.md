...
- [X] T004 [P] Create stub `crates/backup/src/archive.rs` (empty module)
- [X] T005 [P] Create stub `crates/backup/src/checksum.rs` (empty module)
- [X] T006 [P] Create stub `crates/backup/src/paths.rs` (empty module)
- [X] T007 Add `assistant-backup = { path = "../backup", version = "*" }` to `crates/interface-cli/Cargo.toml`
- [X] T008 Verify `cargo check -p assistant-backup` and `cargo check -p assistant-cli` both pass
...
- [X] T047 [P] Write ADR `docs/adr/008-backup-crate.md` documenting: new `assistant-backup` crate, `BackupFs` trait rationale, archive format choice (tar.gz), SQLite WAL checkpoint strategy

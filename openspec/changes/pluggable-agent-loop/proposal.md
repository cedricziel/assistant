...
- `agent-loop-compaction`: Context history compaction following pi-mono's `before_compact`/`compact` event pattern. The `Plugin` trait gains `before_compact` (cancellable) and `on_compact` (notification). `CompactionPlugin` (base crate) summarizes old messages via LLM when `messages.len() > threshold` and contributes a `/compact` tool. Two new `AgentEvent` variants: `CompactionStarted` and `CompactionCompleted`. `MemoryPlugin` gains opt-in `with_compaction(provider)` (compacts oversized memory files at session end when truncation is detected) and `with_reflection(provider)` (extracts session learnings and appends to MEMORY.md + daily notes at session end). Full conversation history is never lost — `StoragePlugin` persists pre-compaction turns.

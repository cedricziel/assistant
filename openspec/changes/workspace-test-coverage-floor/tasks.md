...
- [x] 3.10 Repeat for `assistant-bus-nats`, `assistant-storage` (event timestamps in `conversation_events`, `traces`, `metrics`), and `assistant-llm-provider` (retry backoff via `crates/llm-provider/src/retry.rs`). `NatsMessageBus` gains a clock field; storage's 13 stores each gain `clock` + `with_clock` builder; `OpenAiOAuthManager` migrated; retry backoff jitter switched from `SystemTime::now()` to `getrandom::fill` (proper randomness instead of misusing the clock). Migration of remaining out-of-scope crates (web-ui, interface-cli, otel exporters, tool-executor builtins, backup, workflow, core) lands in 3.10b / 3.10c follow-ups before the lint flips on in 3.11.

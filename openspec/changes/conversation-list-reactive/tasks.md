...
- [x] Remove `refresh()` calls from `ChatNotifier` (after `DoneEvent`, voice completion, event replay). The stream handles updates now. Verify existing chat tests still pass.
